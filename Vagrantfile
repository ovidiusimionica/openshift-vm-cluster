...
Vagrant.configure("2") do |config|
  ...
  config.vm.define "master", primary: true do |node|
    ...
    node.hostmanager.aliases = %w(etcd.example.com)
  
    node.hostmanager.aliases = %w(nfs.example.com)
    # 
	...
	if File.exist?(".vagrant/machines/node03/virtualbox/private_key")
      node.vm.provision "node03-key", type: "file", run: "never", source: ".vagrant/machines/node03/virtualbox/private_key", destination: "/home/vagrant/.ssh/node03.key"
    end
     node.vm.provision "keys", type: "shell", inline: <<-SHELL
